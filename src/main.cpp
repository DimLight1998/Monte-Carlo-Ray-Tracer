...
#pragma omp parallel for default(none) shared(i, n, cam, bvh, dataSum, data)
    ...
    //    cout << 1;
    //    vector<Sphere> balls;
    //    const auto white = make_shared<ConstantColor>(ConstantColor({0.9, 0.9, 0.9}));
    ...
    //    Lambertian lamWhite(make_shared<Texture>(white));
    //    Lambertian lamDark(make_shared<Texture>(dark));
    //    Metal metalDark({0.3, 0.3, 0.3}, 0.7f);
    //    Lambertian lamGray(make_shared<Texture>(gray));
    //    LightSource orangeLight(make_shared<Texture>(orange));
    //
    ...
    //    for_each(balls.begin(), balls.end(), [&](const auto &ball) { hitables.emplace_back(ball); });
