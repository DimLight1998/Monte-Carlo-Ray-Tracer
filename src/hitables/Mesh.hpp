...
class Mesh: public Hitable {
    ...
    explicit Mesh(const std::string& objPath, const std::shared_ptr<Material>& material) {
        ...
        if (!ret) throw std::runtime_error("object load failed: " + err);
    ...
    [[nodiscard]] std::unique_ptr<BVH> BuildBVH() const override {
        ...
        std::for_each(_triangles.begin(), _triangles.end(), [&](const auto& triangle) {
            triangles.emplace_back(std::make_shared<Triangle>(triangle));
        });
